...
def show_instructions(): 
    ...
    with st.container(height=700, border=True):
        ...
        satohuru_items = ["お礼品予備項目", "お礼品ID", "公開フラグ", "全在庫数", "受付開始日", "受付終了日"]

        # タグ風デザインのHTMLを生成
        # インデントによるコードブロック化を防ぐため、1行で記述
        rakuten_tags_html = ""
        for item in rakuten_items:
            rakuten_tags_html += f"""<span style="display: inline-block; padding: 4px 12px; margin: 4px 2px; background-color: #f0f2f6; color: #31333F; border: 1px solid #d1d5db; border-radius: 16px; font-size: 0.85em; font-weight: 500;">{item}</span>"""

        satohuru_tags_html = ""
        for item in satohuru_items:
            satohuru_tags_html += f"""<span style="display: inline-block; padding: 4px 12px; margin: 4px 2px; background-color: #f0f2f6; color: #31333F; border: 1px solid #d1d5db; border-radius: 16px; font-size: 0.85em; font-weight: 500;">{item}</span>"""
        ...
        **① フィルター**
        ...
        > 
        > ・ここでフィルターしたデータのみ、掲載結果に出力されます。「3. 掲載状況」のフィルターでは、こちらで設定したコード以外のデータは設定できません。
